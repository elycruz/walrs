...
mod value_tests {
  ...
  fn async_value_not_null() -> Rule<Value> {
  ...
  #[tokio::test]
  async fn custom_async_value_fails() {
    let rule = async_value_not_null();
    let err = rule.validate_ref_async(&Value::Null).await.unwrap_err();
    assert_eq!(err.message(), "must not be null");
  }

  #[tokio::test]
  async fn custom_async_value_validate_async() {
    let rule = async_value_not_null();
