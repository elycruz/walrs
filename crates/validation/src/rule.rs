...
use crate::{IntoMessage, Message, SteppableValue, Violation};
...
impl<T> Rule<T> {
  ...
  ///
  /// Accepts a static string, a [`Message`], or any type implementing
  /// [`IntoMessage`]. When the inner rule fails, the custom message is
  /// used instead of the default message.
  ///
  /// # Static message
  ///
  /// ```rust
  ...
  /// # Dynamic message
  ///
  /// Use [`Message::provider`] to supply a closure that receives a
  /// [`MessageContext`] containing the value being validated and rule
  /// parameters, enabling rich interpolated error messages.
  ///
  /// ```rust
...
mod tests {
  ...
  #[test]
  fn test_rule_with_message() {
    let rule =
      Rule::<i32>::Min(0).with_message(Message::provider(|ctx| format!("Got {}, expected >= 0.", ctx.value)));
