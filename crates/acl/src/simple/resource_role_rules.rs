...
mod test_resource_role_rules {
  ...
  fn test_get_role_privilege_rules_mut() {
    ...
    assert!(
      rules_mut
        ...
        .unwrap()
        .is_empty(),
      "by_resource_id map should be empty"
    );
    assert!(
      rules_mut.by_role_id.as_ref().unwrap().is_empty(),
      "by_role_id map should be empty"
    );
    ...
    assert_eq!(
      rules_mut.clone(),
      ctrl.by_resource_id.get(resource).unwrap().clone(),
      "mut rules getter should return the same object as the resource's rules"
    );
  }
  ...
  fn test_get_or_create_role_privilege_rules_mut() {
    ...
    // Test with non-existent resource - currently returns for_all_resources (doesn't create)
    {
      ...
      assert!(
        rules_mut
          ...
          .unwrap()
          .is_empty(),
        "by_privilege_id map should be empty"
      );
      assert!(
        rules_mut.by_role_id.as_ref().unwrap().is_empty(),
        "by_role_id map should be empty"
    ...
    assert_eq!(
      ctrl.for_all_resources.for_all_roles.for_all_privileges,
      Rule::Allow,
