  ...
  B: 'static,
{
  ...
  fn call(&self, req: ServiceRequest) -> Self::Future {
    ...
    let allowed = self
      .acl
      .is_allowed(Some(role), Some(resource), Some(privilege));
...
async fn main() -> std::io::Result<()> {
  ...
  let file =
    File::open("test-fixtures/example-extensive-acl-array.json").expect("Failed to open ACL file");

  let acl_data: AclData = serde_json::from_reader(file).expect("Failed to parse ACL JSON");
