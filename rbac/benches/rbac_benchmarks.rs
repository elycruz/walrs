...
fn build_rbac() -> walrs_rbac::Rbac {
  RbacBuilder::new()
    .add_role("guest", &["read.public"], None).unwrap()
    .add_role("user", &["write.post", "comment.post"], Some(&["guest"])).unwrap()
    .add_role("editor", &["edit.post", "publish.post"], Some(&["user"])).unwrap()
    .add_role("admin", &["admin.panel", "manage.users"], Some(&["editor"])).unwrap()
    .build().unwrap()
}
...
criterion_group!(benches, benchmark_build, benchmark_is_granted, benchmark_has_role);
