...
impl<T> Debug for NumberInputConstraints<'_, T>
where
  T: Copy + Display + Eq + Ord,
{
...
where
  T: Debug + Display + Copy + Ord + Eq,
{
  fn validate(&self, option_t: Option<T>) -> Result<(), (ValidationResultEnum, String)> {
    let v_rslt = option_t.map_or(if self.required { ValueMissing } else { Valid }, |v| {
      _validate_number(self, v)
    });
...
mod test {
  use crate::number::{
    range_overflow_msg, range_underflow_msg, value_missing_msg, NumberInputConstraints,
  };
  use crate::types::{
  ...
  fn test_validate_single_threaded() {
    ...
    input_3.custom = Some(Arc::new(&|_: &NumberInputConstraints<i32>,
                                     x: Option<i32>|
     -> bool { x.unwrap_or(0) == 99 }));
    input_3.custom_error = Arc::new(
      &|_: &NumberInputConstraints<i32>, x: Option<i32>| -> String {
        format!(
          "\"{:}\" should equal \"{:}\"",
  ...
  fn test_validate_multi_threaded() {
    ...
    type TestCaseGetter<'a, 'b, T = i32> = &'a (dyn Fn(&NumberInputConstraints<T>) -> (&'b str, Option<i32>, ValidationResult)
           + Send
           + Sync);
