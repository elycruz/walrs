...
mod tests {
    ...
    fn get_char_one_of_validator(haystack: &'static str) -> Box<dyn Fn(char) -> Option<Violation>> {
        Box::new(move |value: char| if haystack.chars().any(|c| c == value) {
            None
        } else {
    ...
    fn even_validator<T>(value: T) -> Option<Violation>
        where T: InputValue + Rem + From<isize>,
    ...
    fn run_validate_test_cases<T: InputValue>(
        ...
        for (i, (value, expected)) in test_values.iter().enumerate() {
            ...
            input.validate(*value).as_deref().map_or_else(
                ...
                |vs| if let Some(rhs_vs) = expected.as_deref() {
                    assert_eq!(vs, rhs_vs);
                } else {
    ...
    fn test_general() {
        ...
        number_input.validators = Some(vec![
            &one_to_ten_validator,
            &even_validator,
        ]);
