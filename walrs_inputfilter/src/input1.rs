...
///
/// ```rust
/// use walrs_inputfilter::{Filter, Input1, Validator, value_missing_msg_getter};
///
/// let input = Input1::<usize, usize, Validator<usize>, Filter<usize>>::new();
///
...
pub fn value_missing_msg_getter<
    ...
    ValidatorFnT: ?Sized,
    FilterFnT: ?Sized
>(_: &Input1<T, FT, ValidatorFnT, FilterFnT>) -> ViolationMessage {
    "Value is missing".to_string()
}

#[derive(Builder, Clone)]
#[builder(setter(strip_option))]
pub struct Input1<'a, T, FT=T, VFn = Validator<T>, FFn = Filter<FT>>
    where T: Copy,
          FT: From<T>,
          VFn: ?Sized,
          FFn: ?Sized
{
    ...
    #[builder(default = "None")]
    pub validators: Option<Vec<&'a VFn>>,

    #[builder(default = "None")]
    pub filters: Option<Vec<&'a FFn>>,

    #[builder(default = "&value_missing_msg_getter")]
    pub value_missing_msg: &'a (dyn Fn(&Input1<'a, T, FT, VFn, FFn>) -> ViolationMessage + Send + Sync)
}

impl<'a, T: Copy, FT: From<T>, VFn, FFn> Input1<'a, T, FT, VFn, FFn>
    where T: Copy,
          FT: From<T>,
          VFn: ?Sized,
          FFn: ?Sized,
...
impl<'a, T, FT: From<T>, VFn: ?Sized, FFn:?Sized> InputConstraints<T, FT> for Input1<'a, T, FT, VFn, FFn>
    where T: Copy,
          FT: From<T>
{
    ...
    /// let usize_required = Input1Builder::<usize, usize, Validator<usize>, Filter<usize>>::default()
    ///   .required(true)
    ...
    ///
    /// // Setup input constraints
    /// let usize_input = Input1Builder::<usize, usize, Validator<usize>, Filter<usize>>::default()
    ...
    /// type ConstraintStruct<'a> = Input1<'a, usize, usize, Validator<usize>, Filter<usize>>;
...
impl<'a, T, FT: From<T>, VFn:?Sized, FFn:?Sized> Default for Input1<'a, T, FT, VFn, FFn>
    where T: Copy,
          FT: From<T>,
{
    ...
    /// ```rust
    /// use walrs_inputfilter::{
    ///   Input1, InputConstraints, ViolationEnum,
    ///   Validator, Filter
    /// };
    ///
    /// type VFn = Validator<usize>;
    /// type FFn = Filter<usize>;
    ///
    /// let input = Input1::<usize, usize, VFn, FFn>::default();
    ///
    /// // Assert defaults
    /// // ----
    /// assert_eq!(input.break_on_failure, false);
...
impl<'a, T, FT: From<T>, VFn, FFn> Display for Input1<'a, T, FT, VFn, FFn>
    where T: Copy,
          FT: From<T>,
          VFn: ?Sized,
          FFn: ?Sized, {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("Input1")
...
impl<'a, T, FT: From<T>, VFn, FFn> Debug for Input1<'a, T, FT, VFn, FFn>
    where T: Copy,
          FT: From<T>,
          VFn: ?Sized,
          FFn: ?Sized, {
...
mod test {
    ...
    // use crate::{Input1Builder, StringConstraintsBuilder};
    // use crate::ViolationEnum::StepMismatch;
    use super::*;

    #[test]
    fn test_new() -> Result<(), Box<dyn Error>> {
        let _ = Input1::<&str, Cow<str>, Validator<&str>, Filter<Cow<str>>>::new();
        let _ = Input1::<char, char, Validator<char>, Filter<char>>::new();
        let _ = Input1::<usize, usize, Validator<usize>, Filter<usize>>::new();
        let _ = Input1::<bool, bool, Validator<bool>, Filter<bool>>::new();
        let _ = Input1::<usize, usize, Validator<usize>, Filter<usize>>::new();
        ...
        let percent = Input1Builder::<usize, usize, Validator<usize>, Filter<usize>>::default()
            .validators(vec![
                &|x| if x != 0 && x % 5 != 0 {
        ...
        let slug_input = Input1Builder::<&str, Cow<str>, Validator<&str>, Filter<Cow<str>>>::default()
            .validators(vec![
    ...
    fn test_validate_detailed() {
        ...
        let usize_input_default = Input1Builder::<usize, usize, Validator<usize>, Filter<usize>>::default()
            .build()
            .unwrap();

        let usize_not_required = Input1Builder::<usize, usize, Validator<usize>, Filter<usize>>::default()
        ...
        let f64_input_required = Input1Builder::<f64, f64, Validator<f64>, Filter<f64>>::default()
            .required(true)
            .validators(vec![
                &zero_to_ten,
                &|x: f64| if x % 2.0 != 0.0 {
        ...
        // Test `char` usage
        let char_input = Input1Builder::<char, char, Validator<char>, Filter<char>>::default()
    ...
    fn test_filter() -> Result<(), Box<dyn Error>> {
        ...
        // ----
        // 1. With no filters.
        let usize_input_default = Input1Builder::<usize, usize, Validator<usize>, Filter<usize>>::default().build()?;

        // 2. With one filter.
        let usize_input_twofold = Input1Builder::<usize, usize, Validator<usize>, Filter<usize>>::default()
            .filters(vec![
                &|x: usize| x * 2usize,
            ])
            .build()?;
        ...
        // 3. With two filters.
        let usize_input_gte_four = Input1Builder::<usize, usize, Validator<usize>, Filter<usize>>::default()
            .filters(vec![
    ...
    fn test_validate_and_filter_detailed() -> Result<(), Box<dyn Error>> {
        ...
        let usize_not_required_with_rules = Input1Builder::<usize, usize, Validator<usize>, Filter<usize>>::default()
            .validators(vec![
                &one_to_ten,
                &validate_is_even,
            ])
