...
impl LocalizedFormValidator {
    ...
    fn validate_username(&self, value: &str) -> Result<(), String> {
        ...
        rule.validate_ref(value, self.locale.as_deref())
    ...
    fn validate_email(&self, value: &str) -> Result<(), String> {
        let rule = Rule::<String>::Required
            ...
            .and(Rule::<String>::Email.with_message_provider(|ctx| match ctx.locale {
    ...
    fn validate_password(&self, value: &str) -> Result<(), Vec<String>> {
        let rule = Rule::<String>::Required
            ...
            .and(
                Rule::<String>::MinLength(8).with_message_provider(|ctx| {
                    let min = 8;
                    match ctx.locale {
                        Some("es") => {
            ...
            .and(
                Rule::<String>::Pattern(r"[A-Z]".to_string()).with_message_provider(|ctx| {
                    match ctx.locale {
                        Some("es") => {
                            "La contraseña debe contener al menos una letra mayúscula".to_string()
                        }
                        Some("fr") => {
                        ...
                        Some("de") => {
                            "Passwort muss mindestens einen Großbuchstaben enthalten".to_string()
                        }
            ...
            .and(
                Rule::<String>::Pattern(r"[0-9]".to_string()).with_message_provider(|ctx| {
                    match ctx.locale {
                        ...
                        Some("de") => {
                            "Passwort muss mindestens eine Zahl enthalten".to_string()
                        }
        ...
        rule.validate_ref_all(value, self.locale.as_deref())
