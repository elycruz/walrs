...
use walrs_form::{
    Form, FormMethod, InputElement, InputType, SelectElement, SelectOption,
    TextareaElement, ButtonElement, ButtonType, FieldsetElement, Element
};
fn main() {
    // Create the form
...
fn print_element(element: &Element, indent: usize) {
    ...
    match element {
        Element::Input(input) => {
            println!("{}Input: {:?} (type: {:?}, required: {:?})", 
                prefix, input.name, input._type, input.required);
        }
        Element::Select(select) => {
            println!("{}Select: {:?} ({} options)", 
                prefix, select.name, select.options.len());
        }
        Element::Textarea(textarea) => {
            println!("{}Textarea: {:?} (rows: {:?})", 
