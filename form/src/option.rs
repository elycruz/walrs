...
const OPTION_INVALID_MSG: &'static str =
...
impl<Value> HTMLOptionCtrlBuilder<Value>
where
  Value: FormControlValue,
{
...
pub fn deselect_all<Value: FormControlValue>(options: &mut [HTMLOptionCtrl<Value>]) {
...
pub fn is_value_in_options<Value: FormControlValue>(
  ...
  options.map_or(false, |ops| {
    if ops.len() == 0 {
      ...
      ops
        .iter()
        .any(|o| o.value.as_ref().zip(v).map_or(false, |(a, b)| a == b))
    }
  })
}
...
pub mod html_option_ctrl_tests {
  ...
  pub fn assert_default_option_ctrl<T>(ctrl: &HTMLOptionCtrl<T>)
  where
    T: FormControlValue,
  {
  ...
  fn test_option_ctrl_builder() -> Result<(), Box<dyn Error>> {
    ...
    match HTMLOptionCtrlBuilder::default().build() as Result<HTMLOptionCtrl<&str>, String> {
      Ok(rslt) => panic!(
  ...
  fn test_option_ctrl_label_and_value_setters() {
    let mut ctrl1: HTMLOptionCtrl<usize> =
      HTMLOptionCtrlBuilder::default().value(98).build().unwrap();
        ...
        .rfold(None as Option<HTMLOptionCtrl<&str>>, |right, (i, _)| {
          ...
          left.options = match right {
            Some(right) => {
              ...
              if i + 1 < VOWELS_LEN - 1 {
                let mut options_tail: Vec<Box<HTMLOptionCtrl<&str>>> = (&VOWELS[i + 1..])
                  .char_indices()
                  .map(|(i2, _)| {
                    Box::new(
                      HTMLOptionCtrlBuilder::default()
