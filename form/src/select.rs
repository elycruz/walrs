...
use crate::constants::{SELECT_ONE_SYMBOL, SELECT_SYMBOL};
use crate::traits::{FormControl, FormControlConstraints, FormControlValue};
use serde::{Deserialize, Serialize};
use std::collections::HashMap;
use std::fmt::{Debug, Display, Formatter};
use walrs_inputconstraints::types::InputConstraints;

use crate::option::{is_value_in_options, HTMLOptionCtrl};
  ...
  ValueConstraints: FormControlConstraints<Value>,
{
  ...
  fn set_value(&mut self, value: Option<Value>) -> bool {
    // @todo Should take `Option<&Value>` here, instead of `Option<Value>`, etc.
    self.value = value;
    self.check_validity()
  }
...
pub mod test {
  use super::*;
  use crate::constants::{
    CONSTRAINTS_SYMBOL, DISABLED_SYMBOL, HELP_MESSAGE_SYMBOL, HTML_ATTRIBS_SYMBOL, ID_SYMBOL,
    LABEL_SYMBOL, MULTIPLE_SYMBOL, NAME_SYMBOL, OPTIONS_SYMBOL, REQUIRED_SYMBOL, TYPE_SYMBOL,
  ...
  fn test_is_value_in_options() {
    ...
    for (i, (test_name, value, options, expected_rslt)) in test_cases.into_iter().enumerate() {
      ...
      let ctrl: HTMLSelectControl<&str, TextInputConstraints> = HTMLSelectControlBuilder::default()
        .options(options)
        .build()
        .unwrap();
  ...
  fn test_html_select_builder() -> Result<(), Box<dyn Error>> {
    ...
    let options: Vec<Box<HTMLOptionCtrl<String>>> = VOWELS_STR
      .chars()
      .enumerate()
      .map(|(i, c)| {
        Box::new(
