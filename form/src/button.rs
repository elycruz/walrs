...
pub mod test {
  use super::*;
  use std::error::Error;
  use walrs_inputconstraints::text::{value_missing_msg, TextInputConstraints};

  #[test]
  fn test_html_button_new() {
    let _with_defaults: HTMLButtonControl<String, TextInputConstraints> =
      HTMLButtonControl::new(None);
  ...
  fn test_html_button_validate() {
      ...
      ); 4]
    {
      let mut input: HTMLButtonControl<&str, TextInputConstraints> =
        HTMLButtonControlBuilder::default().build().unwrap();
