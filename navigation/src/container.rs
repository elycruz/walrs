...
mod tests {
  ...
  fn test_fluent_interface() {
    ...
    nav
      .add_page(Page::builder().label("Home").uri("/").build())
      .add_page(Page::builder().label("About").uri("/about").build())
    ...
    nav
      .set_pages(vec![Page::builder().label("Reset").uri("/reset").build()])
  ...
  fn test_into_iterator() {
    ...
    let labels: Vec<_> = nav.into_iter().filter_map(|p| p.label.clone()).collect();
  ...
  fn test_find_page_mut() {
    ...
    assert_eq!(
      nav.find_by_uri("/").unwrap().label.as_deref(),
      Some("Modified")
    );
  }
