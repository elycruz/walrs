...
mod tests {
  ...
  fn test_page_fluent_interface() {
    ...
    page
      .add_page(Page::builder().label("Books").build())
      .add_page(Page::builder().label("Electronics").build())
    ...
    page
      .set_attribute("data-id", "products")
      .set_attribute("data-section", "main")
      .set_attribute("data-visible", "true");

    assert_eq!(page.attributes.len(), 3);
    assert_eq!(
      page.attributes.get("data-id"),
    ...
    let mut root = Page::builder().label("Root").build();
    root
      .add_page(Page::builder().label("Child").build())
  ...
  fn test_find_page_mut() {
    ...
    let child = root
      .find_page(|p| p.uri.as_deref() == Some("/child"))
      .unwrap();
    assert!(child.active);
  }
  ...
  fn test_clear_pages_fluent() {
    let mut root = Page::builder().label("Root").build();
    root
      .add_page(Page::builder().label("A").build())
  ...
  fn test_set_attributes() {
    ...
    assert_eq!(page.attributes.get("data-id"), Some(&"home".to_string()));
    assert_eq!(
      page.attributes.get("data-section"),
      Some(&"main".to_string())
    );
  }
  ...
  fn test_get_attribute() {
    let mut page = Page::builder().label("Home").build();
    page
      .set_attribute("data-id", "home")
      .set_attribute("data-section", "main");

    assert_eq!(page.get_attribute("data-id"), Some(&"home".to_string()));
    assert_eq!(
      page.get_attribute("data-section"),
      Some(&"main".to_string())
    );
    assert_eq!(page.get_attribute("nonexistent"), None);
  }
  ...
  fn test_set_attributes_fluent() {
    let mut page = Page::builder().label("Home").build();
    let attrs = HashMap::from([("data-id".to_string(), "home".to_string())]);
    ...
    assert_eq!(page.attributes.get("data-id"), Some(&"home".to_string()));
    assert_eq!(
      page.attributes.get("data-extra"),
      Some(&"value".to_string())
    );
    assert_eq!(
      page.attributes.get("data-another"),
  ...
  fn test_clear_attributes_fluent() {
    ...
    page.clear_attributes().set_attribute("data-new", "value");
