...
fn main() -> Result<(), Box<dyn std::error::Error>> {
  ...
  // Create a products section with nested pages using Page fluent interface
  ...
  // Add more root-level pages using Page fluent interface
  ...
  if let Some(books) = nav.find_by_uri("/products/books") {
    println!(
      "Found by URI: {} at /products/books",
      books.label.as_deref().unwrap()
    );
  }
  ...
  if let Some(products_page) = nav.find_by_route("products") {
    println!(
      "Found by route: {}",
      products_page.label.as_deref().unwrap()
    );
  }
  ...
  if let Some(page) = nav.find_page(|p| p.label.as_deref() == Some("Blog")) {
    println!(
      "Found by predicate: {} (target={})",
      page.label.as_deref().unwrap(),
      page.target.as_deref().unwrap_or("none")
    );
  }

  // Find all pages with a specific class using find_page
  let nav_items: Vec<_> = nav
    .pages()
    .iter()
  ...
  println!(
    "Has /products/books? {}",
    nav.has_page(|p| p.uri.as_deref() == Some("/products/books"), true)
  );
  println!();
  ...
  println!(
    "{}",
    view::render_menu_with_class(&nav, "navbar-nav", "current")
  );
  println!();
