...
const OPTION_INVALID_MSG: &'static str = "Expected either `label`, or `value` to be set;  Found 'only' `None`";

#[derive(Serialize, Deserialize, Debug, Builder, Clone)]
#[builder(build_fn(validate = "Self::validate"), setter(into, strip_option))]
pub struct HTMLOptionCtrl<Value>
  where
...
impl<Value> HTMLOptionCtrlBuilder<Value> where
  Value: FormControlValue,
{
  ...
  fn validate(&self) -> Result<(), String> {
    ...
    if (is_option_ctrl && self.value.is_none() &&
      self.label.is_none()) || (!is_option_ctrl && self.label.is_none()) {
...
impl<Value> HTMLOptionCtrl<Value>
  where
    Value: FormControlValue,
{
  pub fn new() -> Self {
...
pub fn deselect_all<Value: FormControlValue>(
  options: &mut [HTMLOptionCtrl<Value>]) {
...
pub fn select_matching_multi<'a, Value: FormControlValue>(
  mut options: &mut [HTMLOptionCtrl<Value>],
  values: &[&Value],
) -> bool {
...
pub fn is_value_in_options<Value: FormControlValue>(
  options: Option<&[Box<HTMLOptionCtrl<Value>>]>,
  v: Option<&Value>,
) -> bool {
  options.map_or(false, |ops|
    if ops.len() == 0 { false } else {
      ops.iter().any(|o|
        o.value.as_ref()
          .zip(v)
...
pub mod html_option_ctrl_tests {
  ...
  use crate::option::{HTMLOptionCtrl, HTMLOptionCtrlBuilder, OPTION_INVALID_MSG};
  use crate::traits::FormControlValue;
  ...
  pub fn options_from_str_provider<'a>(value_seed: &'a str) -> Vec<Box<HTMLOptionCtrl<&'a str>>> {
    let mut ops: Vec<Box<HTMLOptionCtrl<&'a str>>> = vec![];

    for (i, c) in value_seed.char_indices() {
      ops.push(Box::new(HTMLOptionCtrlBuilder::default()
  ...
  pub fn assert_default_option_ctrl<T>(ctrl: &HTMLOptionCtrl<T>) where T: FormControlValue {
  ...
  /// Checks if `optgroup` (`HTMLOptionCtrl` containing `options`), and all it's child options (checked recursively) are "disabled".
  pub fn optgroup_is_disabled<T>(optgroup: &HTMLOptionCtrl<T>, disabled: bool) -> bool
    where T: FormControlValue {
    optgroup.disabled == disabled &&
      optgroup.options.as_deref().map_or(disabled, |os| os.iter().all(|o|
      optgroup_is_disabled(o, disabled)))
  }

  #[test]
  fn test_option_ctrl_new() {
    let ctrl: HTMLOptionCtrl<usize> = HTMLOptionCtrl::new();
  ...
  fn test_option_ctrl_builder() -> Result<(), Box<dyn Error>> {
    ...
    let invalid_option: Result<HTMLOptionCtrl<usize>, String> =
      HTMLOptionCtrlBuilder::default().build();

    match invalid_option {
      Ok(option) => panic!(
        "Builder should return an `Err(String)` when both \
        `value` and `label` are `None`;  Received {:?}", option),
    ...
    let test_cases: Vec<(
      &'static str,
      HTMLOptionCtrl<usize>,
      (Option<usize>,
       Option<usize>,
       bool, bool
      ))> = vec![
      ("With Value and Label (1)", HTMLOptionCtrlBuilder::default()
        .value(99) // Should populate `label` as well
        .build()?,
       (Some(99), Some(99), !DISABLED, !SELECTED)
      ),
      ("With Value and Label (2)", HTMLOptionCtrlBuilder::default()
        .label(99) // Should populate `value`, as well
        .build()?,
       (Some(99), Some(99), !DISABLED, !SELECTED)
      ),
      ("With Value, Label, and selected", HTMLOptionCtrlBuilder::default()
    ...
    match HTMLOptionCtrlBuilder::default()
      .build() as Result<HTMLOptionCtrl<&str>, String> {
      Ok(rslt) =>
        panic!("Expected builder 'build()' call to produce an `Err(String)`;  \
        Received {:?}", rslt),
      Err(_) => ()
    }
  ...
  fn test_option_ctrl_label_and_value_setters() {
    let mut ctrl1: HTMLOptionCtrl<usize> = HTMLOptionCtrlBuilder::default()
      .value(98)
      .build()
      .unwrap();
    ...
    assert_eq!(*ctrl1.get_value().unwrap(), 100, "`get_value()` should always return either `value`\
  ...
  fn test_option_ctrl_get_value() {
    let mut ctrl: HTMLOptionCtrl<usize> = HTMLOptionCtrl::new();
    ...
    ctrl.label = Some(99);
    assert_eq!(ctrl.get_value(), ctrl.label.as_ref(), "When `value` is not set, should return `label`'s value");

    // When `value` is set
    ctrl.value = Some(100);
    assert_eq!(ctrl.get_value(), ctrl.value.as_ref(), "When `value` is set, should return `value`'s value");

    // When `value` is unset
    ctrl.value = None;
    assert_eq!(ctrl.label, Some(99));
    assert_eq!(ctrl.get_value(), ctrl.label.as_ref(), "When `value` is 'unset', should return `label`'s value");

    // When neither, `value`, or `label`, is set
    ctrl.label = None;
    assert_eq!(ctrl.get_value(), None, "When neither, `label`, and/or, `value`, is set, should return `None`");
  }

  #[test]
  fn test_option_ctrl_set_disabled() {
    let mut ctrl: HTMLOptionCtrl<&str> = HTMLOptionCtrl::new();
    assert_default_option_ctrl(&ctrl);
    ...
    match ctrl.options.as_deref() {
      Some(ops) =>
        assert!(ops.iter().all(|o| !o.disabled), "Options should not be disabled by default"),
      None => panic!("Expect 'Vec<Box<HTMLOptionCtrl>>' found 'None'")
    }

    ctrl.set_disabled(true);
    assert_eq!(ctrl.disabled, true);

    assert!(optgroup_is_disabled(&ctrl, true), "Expected 'optgroup' to be `disabled`");
  }

  #[test]
  fn test_option_ctrl_as_opt_group() -> Result<(), Box<dyn Error>> {
    let mut opt_group = VOWELS.char_indices()
      .rfold(None as Option<HTMLOptionCtrl<&str>>, |right, (i, _)| {
        let mut left: HTMLOptionCtrl<&str> = HTMLOptionCtrlBuilder::default()
          .label(&VOWELS[i..i + 1])
          .build().unwrap();

        left.options = match right {
          Some(right) => {
            let mut options: Vec<Box<HTMLOptionCtrl<&str>>> = vec![Box::new(right)];

            if i + 1 < VOWELS_LEN - 1 {
              let mut options_tail: Vec<Box<HTMLOptionCtrl<&str>>> =
                (&VOWELS[i + 1..])
                  .char_indices()
                  .map(|(i2, _)| {
                    Box::new(HTMLOptionCtrlBuilder::default()
    ...
    fn assert_nested_optgroup_structure(_og: &HTMLOptionCtrl<&str>) {
      _og.options.as_ref().map(|ops| {
        println!("Asserting first childs are opt_groups, in opt_group {:?}", &_og.label);
        if ops[0].value.unwrap() != "u" {
          assert!(ops[0].options.is_some(), "Expected first control in list to be an 'opt_group'");
        }
        assert_nested_optgroup_structure(&ops[0])
      });
    }

    fn assert_optgroup_disabled_state(_og: &HTMLOptionCtrl<&str>, disabled: bool) -> bool {
      println!("Asserting disabled state `{}` for optgroup {:?}", disabled, _og.label);
    ...
    opt_group.as_ref().map(|_og| assert_optgroup_disabled_state(_og, false));
