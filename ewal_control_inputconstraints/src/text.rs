...
fn _validate(constraints: &TextInputConstraints, x: Option<&str>) -> Result<(), (ValidationResultEnum, String)> {
  let v_rslt = match x {
...
impl Display for TextInputConstraints<'_> {
  fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
    write!(
      ...
      self.min_length.map(|x| format!("Some({})", x)).unwrap_or("None".into()),
      self.max_length.map(|x| format!("Some({})", x)).unwrap_or("None".into()),
      self.pattern.as_ref().map(|x| format!("Some({})", x)).unwrap_or("None".into()),
      self.required,
      if self.custom.is_some() { "Some(dyn fn(&str) -> bool + Send + Sync)" } else { "None" },
    )
  }
}
