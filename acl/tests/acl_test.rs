    ...
    if let Some(rps) = role_privileges.as_ref() {
      for (role, privileges) in rps.iter() {
        if let Some(privilege_list) = privileges.as_ref() {
          ...
          eprintln!("Testing acl.is_allowed({:?}, {:?}, None)", role, resource);
          assert!(
            acl.is_allowed(Some(role.as_ref()), Some(resource.as_ref()), None),
...
pub fn test_e2e_example_one() -> Result<(), Box<dyn std::error::Error>> {
  ...
  assert!(
    ...
    "admin should inherit from guest (transitively)"
  );
  assert!(
    acl.inherits_role("super-admin", "admin"),
    "super-admin should inherit from admin"
  );
  assert!(
    acl.inherits_role("super-admin", "special-user"),
    "super-admin should inherit from special-user"
  );
  ...
  assert!(
    acl.inherits_resource("blog", "index"),
    "blog should inherit from index"
  );
  ...
  assert!(
    acl.is_allowed(Some("guest"), Some("index"), None),
    "guest should be allowed all privileges on index"
  );
  assert!(
    acl.is_allowed(Some("user"), Some("index"), None),
    "user should be allowed all privileges on index"
  );

  // Test "user" resource privileges
  assert!(
    acl.is_allowed(Some("guest"), Some("user"), Some("index")),
    "guest should have index privilege on user resource"
  );
  assert!(
    acl.is_allowed(Some("guest"), Some("user"), Some("read")),
    "guest should have read privilege on user resource"
  );
  assert!(
    acl.is_allowed(Some("user"), Some("user"), Some("update")),
    "user should have update privilege on user resource"
  );
  assert!(
    acl.is_allowed(Some("admin"), Some("user"), Some("create")),
    "admin should have create privilege on user resource"
  );
  assert!(
    acl.is_allowed(Some("super-admin"), Some("user"), Some("delete")),
    "super-admin should have delete privilege on user resource"
  );

  // Test "account" resource privileges
  assert!(
    acl.is_allowed(Some("guest"), Some("account"), Some("index")),
    "guest should have index privilege on account"
  );
  assert!(
    acl.is_allowed(Some("guest"), Some("account"), Some("read")),
    "guest should have read privilege on account"
  );
  assert!(
    acl.is_allowed(Some("user"), Some("account"), Some("update")),
    "user should have update privilege on account"
  );
  assert!(
    acl.is_allowed(Some("admin"), Some("account"), Some("create")),
    "admin should have create privilege on account"
  );
  assert!(
    acl.is_allowed(Some("super-admin"), Some("account"), Some("delete")),
    "super-admin should have delete privilege on account"
  );

  // Test "blog" resource privileges
  assert!(
    acl.is_allowed(Some("guest"), Some("blog"), Some("index")),
    "guest should have index privilege on blog"
  );
  assert!(
    acl.is_allowed(Some("guest"), Some("blog"), Some("read")),
    "guest should have read privilege on blog"
  );
  assert!(
  ...
  assert!(
    acl.is_allowed(Some("admin"), Some("users"), None),
    "admin should have all privileges on users"
  );

  // Test "rest-resource" privileges
  assert!(
    acl.is_allowed(Some("guest"), Some("rest-resource"), Some("get")),
    "guest should have get privilege on rest-resource"
  );
  assert!(
    acl.is_allowed(Some("special-user"), Some("rest-resource"), Some("get")),
    "special-user should have get privilege on rest-resource"
  );
  assert!(
    acl.is_allowed(Some("user"), Some("rest-resource"), Some("post")),
    "user should have post privilege on rest-resource"
  );
  assert!(
    acl.is_allowed(Some("admin"), Some("rest-resource"), Some("put")),
    "admin should have put privilege on rest-resource"
  );
  assert!(
    acl.is_allowed(Some("super-admin"), Some("rest-resource"), Some("delete")),
    "super-admin should have delete privilege on rest-resource"
  );
  ...
  assert!(
    acl.is_allowed(
      Some("special-user"),
      Some("non/conforming/pathname"),
      Some("further/sub/path")
    ),
  ...
  assert!(
    !acl.is_allowed(Some("guest"), Some("users"), None),
    "guest should be denied all privileges on users"
  );
  assert!(
    !acl.is_allowed(Some("user"), Some("users"), None),
    "user should be denied all privileges on users"
  );
  ...
  assert!(
    !acl.is_allowed(Some("non-existent-role"), Some("account"), Some("read")),
    "non-existent role should not be allowed on account resource"
  );
  assert!(
    !acl.is_allowed(Some("guest"), Some("non-existent-resource"), None),
