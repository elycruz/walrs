...
fn test_acl_builder_complex_scenario() -> Result<(), String> {
  let acl = AclBuilder::new()
    ...
    // Resources
    .add_resources(&[("public", None), ("private", None), ("admin_area", None)])?
    // Rules
...
fn test_acl_builder_add_role_with_multiple_parents() -> Result<(), String> {
  ...
  assert!(acl.has_role("admin"), "Should have admin role");
  assert!(
    acl.inherits_role("admin", "viewer"),
    "Admin should inherit from viewer"
  );
  assert!(
    acl.inherits_role("admin", "editor"),
...
fn test_acl_builder_add_role_with_nonexistent_parent() -> Result<(), String> {
  ...
  assert!(acl.has_role("user"), "Should have user role");
  assert!(
    acl.has_role("nonexistent-parent"),
    "Should auto-create nonexistent-parent role"
  );
  assert!(
    acl.inherits_role("user", "nonexistent-parent"),
    "User should inherit from nonexistent-parent"
  );
  assert_eq!(acl.role_count(), 2, "Should have exactly 2 roles");
...
fn test_acl_builder_add_role_duplicate() -> Result<(), String> {
  ...
  assert_eq!(
    acl.role_count(),
    1,
    "Should have exactly 1 role (not duplicated)"
  );
...
fn test_acl_builder_add_role_chaining() -> Result<(), String> {
  ...
  assert!(
    acl.inherits_role("super_admin", "admin"),
    "Super admin should inherit from admin"
  );

  // Verify transitive inheritance
  assert!(
    acl.inherits_role("super_admin", "guest"),
    "Super admin should transitively inherit from guest"
  );
...
fn test_acl_builder_add_roles_basic() -> Result<(), String> {
  let acl = AclBuilder::new()
    .add_roles(&[("guest", None), ("user", None), ("admin", None)])?
    .build()?;
...
fn test_acl_builder_add_roles_with_single_parent() -> Result<(), String> {
  ...
  assert!(
    acl.inherits_role("admin", "guest"),
    "Admin should transitively inherit from guest"
  );
...
fn test_acl_builder_add_roles_with_multiple_parents() -> Result<(), String> {
  ...
  assert!(
    acl.inherits_role("admin", "moderator"),
    "Admin should inherit from moderator"
  );
  assert!(
    acl.inherits_role("super_admin", "admin"),
    "Super admin should inherit from admin"
  ...
  assert!(
    acl.inherits_role("super_admin", "viewer"),
    "Super admin should inherit from viewer"
  );

  // Verify transitive inheritance
  assert!(
    acl.inherits_role("super_admin", "editor"),
    "Super admin should transitively inherit from editor"
  );
  assert!(
    acl.inherits_role("super_admin", "moderator"),
    "Super admin should transitively inherit from moderator"
  );
...
fn test_acl_builder_add_roles_empty_list() -> Result<(), String> {
  let acl = AclBuilder::new().add_roles(&[])?.build()?;
...
fn test_acl_builder_add_roles_duplicate_roles() -> Result<(), String> {
  let acl = AclBuilder::new()
    .add_roles(&[("guest", None), ("user", Some(&["guest"]))])?
    .add_roles(&[
...
fn test_acl_builder_add_roles_mixed_with_and_without_parents() -> Result<(), String> {
  ...
  assert!(
    !acl.inherits_role("special", "guest"),
    "Special should not inherit from guest"
  );
...
fn test_acl_builder_add_roles_complex_diamond_inheritance() -> Result<(), String> {
  ...
  assert_eq!(acl.role_count(), 4, "Should have exactly 4 roles");
  assert!(
    acl.inherits_role("branch_a", "root"),
    "Branch A should inherit from root"
  );
  assert!(
    acl.inherits_role("branch_b", "root"),
    "Branch B should inherit from root"
  );
  assert!(
    acl.inherits_role("leaf", "branch_a"),
    "Leaf should inherit from branch A"
  );
  assert!(
    acl.inherits_role("leaf", "branch_b"),
    "Leaf should inherit from branch B"
  );
  assert!(
    acl.inherits_role("leaf", "root"),
    "Leaf should transitively inherit from root"
  );
...
fn test_acl_builder_add_resource_with_single_parent() -> Result<(), String> {
  ...
  assert!(
    acl.inherits_resource("blog", "cms"),
    "Blog should inherit from cms"
  );
  assert!(
    !acl.inherits_resource("cms", "blog"),
    "CMS should not inherit from blog"
  );
...
fn test_acl_builder_add_resource_with_multiple_parents() -> Result<(), String> {
  ...
  assert!(
    acl.inherits_resource("document", "readable"),
    "Document should inherit from readable"
  );
  assert!(
    acl.inherits_resource("document", "writable"),
    "Document should inherit from writable"
  );
...
fn test_acl_builder_add_resource_with_transitive_inheritance() -> Result<(), String> {
  ...
  assert!(
    acl.inherits_resource("leaf", "intermediate"),
    "Leaf should inherit from intermediate"
  );
  assert!(
    acl.inherits_resource("leaf", "base"),
    "Leaf should inherit from base (transitively)"
  );
  assert!(
    acl.inherits_resource("intermediate", "base"),
    "Intermediate should inherit from base"
  );
...
fn test_acl_builder_add_resource_chained_calls() -> Result<(), String> {
  ...
  assert!(
    acl.inherits_resource("level4", "level3"),
    "Level4 should inherit from level3"
  );
  assert!(
    acl.inherits_resource("level4", "level2"),
    "Level4 should inherit from level2 (transitive)"
  );
  assert!(
    acl.inherits_resource("level4", "level1"),
    "Level4 should inherit from level1 (transitive)"
  );
...
fn test_acl_builder_add_resource_duplicate() -> Result<(), String> {
  ...
  assert_eq!(
    acl.resource_count(),
    1,
    "Should have exactly 1 resource (not duplicated)"
  );
...
fn test_acl_builder_add_resources_basic() -> Result<(), String> {
  let acl = AclBuilder::new()
    .add_resources(&[("blog", None), ("forum", None), ("wiki", None)])?
    .build()?;
...
fn test_acl_builder_add_resources_with_single_parent() -> Result<(), String> {
  ...
  assert!(
    acl.inherits_resource("article", "content"),
    "Article should inherit from content"
  );
  assert!(
    acl.inherits_resource("blog_post", "article"),
    "Blog post should inherit from article"
  );
  assert!(
    acl.inherits_resource("blog_post", "content"),
    "Blog post should transitively inherit from content"
  );
...
fn test_acl_builder_add_resources_with_multiple_parents() -> Result<(), String> {
  let acl = AclBuilder::new()
    .add_resources(&[
      ...
      (
        "admin_resource",
        Some(&["viewable", "editable", "deletable"]),
      ),
    ])?
    .build()?;
  ...
  assert_eq!(acl.resource_count(), 4, "Should have exactly 4 resources");
  assert!(
    acl.inherits_resource("admin_resource", "viewable"),
    "Admin resource should inherit from viewable"
  );
  assert!(
    acl.inherits_resource("admin_resource", "editable"),
    "Admin resource should inherit from editable"
  );
  assert!(
    acl.inherits_resource("admin_resource", "deletable"),
    "Admin resource should inherit from deletable"
  );
...
fn test_acl_builder_add_resources_empty_list() -> Result<(), String> {
  let acl = AclBuilder::new().add_resources(&[])?.build()?;
...
fn test_acl_builder_add_resources_out_of_order_dependencies() -> Result<(), String> {
  ...
  assert_eq!(acl.resource_count(), 4, "Should have exactly 4 resources");
  assert!(
    acl.inherits_resource("level2", "level1"),
    "Level2 should inherit from level1"
  );
  assert!(
    acl.inherits_resource("level3", "level2"),
    "Level3 should inherit from level2"
  );
  assert!(
    acl.inherits_resource("level4", "level3"),
    "Level4 should inherit from level3"
  ...
  assert!(
    acl.inherits_resource("level4", "level1"),
    "Level4 should transitively inherit from level1"
  );
...
fn test_acl_builder_add_resources_duplicate() -> Result<(), String> {
  let acl = AclBuilder::new()
    .add_resources(&[("blog", None), ("forum", None)])?
    .add_resources(&[
      ("blog", None),  // Duplicate
...
fn test_acl_builder_add_resources_mixed_with_and_without_parents() -> Result<(), String> {
  ...
  assert!(
    acl.inherits_resource("blog", "public"),
    "Blog should inherit from public"
  );
  assert!(
    acl.inherits_resource("admin_blog", "blog"),
    "Admin blog should inherit from blog"
  );
  assert!(
    acl.inherits_resource("admin_blog", "private"),
    "Admin blog should inherit from private"
  );
  assert!(
    !acl.inherits_resource("secret", "public"),
    "Secret should not inherit from public"
  );
...
fn test_acl_builder_add_resources_complex_diamond_inheritance() -> Result<(), String> {
  ...
  assert_eq!(acl.resource_count(), 4, "Should have exactly 4 resources");
  assert!(
    acl.inherits_resource("path_a", "root"),
    "Path A should inherit from root"
  );
  assert!(
    acl.inherits_resource("path_b", "root"),
    "Path B should inherit from root"
  );
  assert!(
    acl.inherits_resource("leaf", "path_a"),
    "Leaf should inherit from path A"
  );
  assert!(
    acl.inherits_resource("leaf", "path_b"),
    "Leaf should inherit from path B"
  );
  assert!(
    acl.inherits_resource("leaf", "root"),
    "Leaf should transitively inherit from root"
  );
...
fn test_acl_builder_to_acl_data_conversion() -> Result<(), String> {
  ...
  assert!(
    roles.iter().any(|(name, _)| name == "guest"),
    "Should contain guest role"
  );
  assert!(
    roles.iter().any(|(name, _)| name == "user"),
    "Should contain user role"
  );
  assert!(
    roles.iter().any(|(name, _)| name == "admin"),
  ...
  let user_role = roles
    ...
    .expect("Should find user role");
  assert!(
    user_role
      .1
      .as_ref()
      .map(|parents| parents.contains(&"guest".to_string()))
      .unwrap_or(false),
    "User should inherit from guest"
  );
  ...
  assert!(
    resources.iter().any(|(name, _)| name == "blog"),
    "Should contain blog resource"
  );
  assert!(
    resources.iter().any(|(name, _)| name == "comment"),
    "Should contain comment resource"
  );
  assert!(
    resources.iter().any(|(name, _)| name == "admin_panel"),
    "Should contain admin_panel resource"
  );
  ...
  let comment = resources
    ...
    .expect("Should find comment resource");
  assert!(
    comment
      .1
      .as_ref()
      .map(|parents| parents.contains(&"blog".to_string()))
      .unwrap_or(false),
...
fn test_acl_builder_to_acl_data_empty() -> Result<(), String> {
  ...
  assert!(
    acl_data.roles.is_none(),
    "Empty builder should have no roles"
  );
  assert!(
    acl_data.resources.is_none(),
    "Empty builder should have no resources"
  );
  assert!(
    acl_data.allow.is_none(),
    "Empty builder should have no allow rules"
  );
  assert!(
    acl_data.deny.is_none(),
    "Empty builder should have no deny rules"
  );
...
fn test_acl_builder_to_acl_data_global_allow_all() -> Result<(), String> {
  ...
  assert!(global_allow.is_some(), "Should have global allow rules (*)");
...
fn test_acl_builder_to_acl_data_deny_all_roles_all_privileges_on_resource() -> Result<(), String> {
  ...
  let acl_data = AclData {
    ...
    allow: Some(vec![(
      ...
      Some(vec![("*".to_string(), Some(vec!["read".to_string()]))]),
    )]),
...
fn test_acl_builder_detects_role_self_cycle() {
  let result = AclBuilder::new()
    .add_role("self_ref", None)
    .unwrap()
    .add_role("self_ref", Some(&["self_ref"]))
    .unwrap()
    .build();

  assert!(
    result.is_err(),
    "Should detect self-referencing cycle in roles"
  );
}
...
fn test_acl_builder_detects_role_circular_dependency() {
  let result = AclBuilder::new()
    .add_role("role_a", Some(&["role_b"]))
    .unwrap()
    .add_role("role_b", Some(&["role_c"]))
    .unwrap()
    .add_role("role_c", Some(&["role_a"]))
    .unwrap()
    .build();

  assert!(
    result.is_err(),
    "Should detect circular dependency in roles"
  );
}
...
fn test_acl_builder_detects_resource_self_cycle() {
  let result = AclBuilder::new()
    .add_resource("self_ref", None)
    .unwrap()
    .add_resource("self_ref", Some(&["self_ref"]))
    .unwrap()
    .build();

  assert!(
    result.is_err(),
    "Should detect self-referencing cycle in resources"
  );
}
...
fn test_acl_builder_detects_resource_circular_dependency() {
  let result = AclBuilder::new()
    .add_resource("res_a", Some(&["res_b"]))
    .unwrap()
    .add_resource("res_b", Some(&["res_c"]))
    .unwrap()
    .add_resource("res_c", Some(&["res_a"]))
    .unwrap()
    .build();

  assert!(
    result.is_err(),
    "Should detect circular dependency in resources"
  );
}
