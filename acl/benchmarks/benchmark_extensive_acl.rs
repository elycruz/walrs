...
fn main() -> Result<(), String> {
  ...
  let acl_data: AclData =
    serde_json::from_reader(file).map_err(|e| format!("Failed to parse ACL JSON: {}", e))?;
  ...
  println!(
    "  - Estimated ACL structure size: ~{} KB",
    estimate_acl_size(&acl) / 1024
  );
  println!();
...
fn run_role_hierarchy_benchmark(acl: &Acl) {
  ...
  println!(
    ...
    hierarchy.len() * (hierarchy.len() - 1) / 2
  );
  println!("  Time: {:?}", duration);
  println!();
}
