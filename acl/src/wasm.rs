...
impl JsAcl {
  ...
  pub fn to_json(&self) -> Result<String, JsValue> {
    ...
    Err(JsValue::from_str(
      "Direct ACL serialization not yet implemented. Use AclBuilder instead.",
    ))
  }
  ...
  pub fn is_allowed(
    ...
    self
      .inner
      .is_allowed(role.as_deref(), resource.as_deref(), privilege.as_deref())
  }
...
impl JsAclBuilder {
  ...
  pub fn from_json(json: &str) -> Result<JsAclBuilder, JsValue> {
    ...
    let builder = AclBuilder::try_from(&acl_data).map_err(|e| JsValue::from_str(&e))?;
  ...
  pub fn add_role(mut self, role: &str, parents: Option<Vec<String>>) -> Self {
    let parent_refs: Option<Vec<&str>> = parents
      .as_ref()
      .map(|p| p.iter().map(|s| s.as_str()).collect());

    self
      .inner
      .add_role(role, parent_refs.as_deref())
  ...
  pub fn add_roles(mut self, roles: Vec<JsValue>) -> Self {
  ...
  pub fn add_resource(mut self, resource: &str, parents: Option<Vec<String>>) -> Self {
    let parent_refs: Option<Vec<&str>> = parents
      .as_ref()
      .map(|p| p.iter().map(|s| s.as_str()).collect());
    ...
    self
      .inner
      .add_resource(resource, parent_refs.as_deref())
      .unwrap_or_else(|e| panic!("{}", e));
  ...
  pub fn add_resources(mut self, resources: Vec<JsValue>) -> Self {
    use serde_wasm_bindgen::from_value;
  ...
  pub fn build(mut self) -> Result<JsAcl, JsValue> {
    let acl = self.inner.build().map_err(|e| JsValue::from_str(&e))?;
