...
fn main() {
    ...
    println!("   Required rule on 'hello': {:?}", required.validate_ref("hello"));
    println!("   Required rule on '': {:?}", required.validate_ref(""));
    println!("   MinLength(3) on 'hi': {:?}", min_length.validate_ref("hi"));
    println!("   MinLength(3) on 'hello': {:?}", min_length.validate_ref("hello"));
    ...
    println!("   Combined rule on 'ab': {:?}", username_rule.validate_ref("ab"));
    println!("   Combined rule on 'john': {:?}", username_rule.validate_ref("john"));
    println!("   Combined rule on 'this_username_is_way_too_long': {:?}",
             username_rule.validate_ref("this_username_is_way_too_long"));
    ...
    println!("   'password': {:?}", password_rule.validate_ref("password"));
    println!("   'Password123': {:?}", password_rule.validate_ref("Password123"));
    ...
    println!("   'user@example.com': {:?}", contact_rule.validate_ref("user@example.com"));
    ...
    println!("   'user@example.com': {:?}", flexible_id.validate_ref("user@example.com"));
    ...
    println!("   not(MinLength(1)) on '': {:?}", is_empty.validate_ref(""));
    println!("   not(MinLength(1)) on 'hello': {:?}", is_empty.validate_ref("hello"));
    ...
    println!("   MinLength(8).when(IsNotEmpty) on '': {:?}", conditional_rule.validate_ref(""));
    println!("   MinLength(8).when(IsNotEmpty) on 'short': {:?}", conditional_rule.validate_ref("short"));
    println!("   MinLength(8).when(IsNotEmpty) on 'longenough': {:?}", conditional_rule.validate_ref("longenough"));
    ...
    println!("   Email on 'test@example.com': {:?}", email_pattern.validate_ref("test@example.com"));
    println!("   Email on 'invalid': {:?}", email_pattern.validate_ref("invalid"));
    println!("   Url on 'https://example.com': {:?}", url_pattern.validate_ref("https://example.com"));
    println!("   Pattern([a-z]+) on 'hello': {:?}", custom_pattern.validate_ref("hello"));
    println!("   Pattern([a-z]+) on 'Hello': {:?}", custom_pattern.validate_ref("Hello"));

    // Example 10: Numeric rules
    println!("\n10. Numeric rules:");
    let age_rule = Rule::<i32>::Min(0).and(Rule::Max(150));
    let percentage_rule = Rule::<f64>::Range { min: 0.0, max: 100.0 };

    println!("   Age rule on -5: {:?}", age_rule.validate(-5));
    println!("   Age rule on 25: {:?}", age_rule.validate(25));
    println!("   Age rule on 200: {:?}", age_rule.validate(200));
    println!("   Percentage on 50.5: {:?}", percentage_rule.validate(50.5));
    println!("   Percentage on 150.0: {:?}", percentage_rule.validate(150.0));
    ...
    println!("   Status 'active': {:?}", status_rule.validate_ref("active"));
    println!("   Status 'invalid': {:?}", status_rule.validate_ref("invalid"));

    // Example 13: Custom messages
    println!("\n13. Custom error messages:");
    let with_message = Rule::<String>::MinLength(8)
        .with_message("Password must be at least 8 characters");

    match with_message.validate_ref("short") {
        Ok(()) => println!("   Valid"),
    ...
    match strict_rule.validate_ref_all("abc") {
