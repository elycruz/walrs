...
fn main() {
    ...
    let username_field = FieldBuilder::<String>::default()
        .name("username")
        .rule(Rule::Required.and(Rule::MinLength(3)).and(Rule::MaxLength(20)))
        .build()
        .unwrap();
    ...
    println!("   break_on_failure matches: {}", original.break_on_failure == deserialized.break_on_failure);
    ...
    println!("   Valid data: {:?}", loaded_filter.validate(&valid_data).is_ok());
    println!("   Invalid data (missing username): {:?}", loaded_filter.validate(&invalid_data).is_ok());
