...
fn main() {
    ...
    form_violations.add_field_violation(
        "password",
        Violation::new(ViolationType::PatternMismatch, "Password must contain a number"),
    );
    ...
    for field_name in form_violations.field_names() {
        if let Some(violations) = form_violations.for_field(field_name) {
            println!("   Field '{}' has {} violation(s):", field_name, violations.len());
    ...
    username_violations.push(Violation::new(ViolationType::PatternMismatch, "Invalid characters"));

    bulk_violations.add_field_violations("username", username_violations);

    println!("   Username violations: {}",
             bulk_violations.for_field("username").map(|v| v.len()).unwrap_or(0));

    // Example 4: Merging FormViolations
    println!("\n4. Merging FormViolations:");
    let mut violations_a = FormViolations::new();
    violations_a.add_field_violation("email", Violation::new(ViolationType::ValueMissing, "Required"));

    let mut violations_b = FormViolations::new();
    violations_b.add_field_violation("phone", Violation::new(ViolationType::ValueMissing, "Required"));
    ...
    println!("   After merge:");
    println!("      Violations A: {} total", violations_a.len());
    println!("      Fields with violations: {:?}", violations_a.field_names().collect::<Vec<_>>());
    ...
    println!("   Non-empty violations -> Result: {:?}", result_err.is_ok());
