...
pub type StrValueMissingViolationCallback =
dyn Fn(&StrInput) -> ViolationMessage + Send + Sync;
...
pub struct StrInput<'a, 'b>
{
    ...
    #[builder(default = "&str_value_missing_msg")]
    pub value_missing: &'a (dyn Fn(&StrInput<'a, 'b>) -> ViolationMessage + Send + Sync),
...
impl<'a, 'b> StrInput<'a, 'b>
{
    pub fn new(name: Option<&'a str>) -> Self {
        StrInput {
            ...
            validators: None,
            filters: None,
            value_missing: &str_value_missing_msg,
        }
    }
}
...
pub fn str_value_missing_msg(_: &StrInput) -> String {
...
mod test {
    ...
    fn test_thread_safety_with_scoped_threads_and_closures() -> Result<(), Box<dyn Error>> {
        ...
        thread::scope(|scope| {
            ...
            scope.spawn(
                || match less_than_input_instance.validate_and_filter(Some("1989-01-01")) {
                    Err(err) => panic!("Expected `Ok(Some({:#?})`;  Received `Err({:#?})`",
                                       Cow::<str>::Owned("1989-01-31".to_string()), err),
            ...
            scope.spawn(
                || match ymd_check_input_instance.validate(Some(&"")) {
                    Err(x) => {
    ...
    fn test_validate_and_filter() {
        ...
        assert_eq!(input.validate_and_filter(Some("2023-12-31")), Err(vec![(RangeOverflow, less_than_1990_msg("2023-12-31"))]));
        assert_eq!(input.validate_and_filter(Some("1989-01-01")), Ok(Some(Cow::Owned("1989-01-31".to_string()))));
    }
