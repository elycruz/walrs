...
use crate::{ViolationEnum, ViolationTuple, ValidationResult, Validator, ViolationMessage, ValidateValue};

pub type StrValidatorViolationCallback = dyn Fn(&StrValidator, &str) -> ViolationMessage + Send + Sync;

pub fn pattern_mismatch_msg(rules: &StrValidator, xs: &str) -> String {
    format!(
...
pub fn too_short_msg(rules: &StrValidator, xs: &str) -> String {
    format!(
...
pub fn too_long_msg(rules: &StrValidator, xs: &str) -> String {
    format!(
...
pub struct StrValidator<'a, 'b> {
    ...
    pub too_short_msg: &'a StrValidatorViolationCallback,

    #[builder(default = "&too_long_msg")]
    pub too_long_msg: &'a StrValidatorViolationCallback,

    #[builder(default = "&pattern_mismatch_msg")]
    pub pattern_mismatch_msg: &'a StrValidatorViolationCallback,
}

impl<'a, 'b> StrValidator<'a, 'b> {
    pub fn new() -> Self {
        StrValidator {
    ...
    fn _validate_against_constraints(&self, value: &'b str) -> ValidationResult {
        let mut errs = vec![];
...
impl<'a> ValidateValue<&'a str> for StrValidator<&'a str> {
    ...
    /// };
    ///
    /// let str_input = StrValidatorBuilder::default()
    ///  .required(true)
    ...
    /// ```
    fn validate(&self, value: &'a str) -> ValidationResult {
        let mut errs = vec![];
        ...
        if let Some(pattern) = &self.pattern {
            if !pattern.is_match(value) {
                errs.push((
                    ViolationEnum::PatternMismatch,
                    (self.pattern_mismatch_msg)(self, value),
                ));
    ...
    /// ```
    fn validate_detailed(&self, value: &'b str) ->  Result<(), Vec<ViolationTuple>> {
        match value {
            None => {
                if self.required {
                    Err(vec![(
            ...
            Some(v) => match self._validate_against_constraints(v) {
...
impl Default for StrValidator<'_, '_> {
    fn default() -> Self {
        Self::new()
    }
}

impl Display for StrValidator<'_, '_> {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "StrValidator {{ break_on_failure: {}, min_length: {}, max_length: {}, pattern: {}, required: {}, validators: {}, filters: {} }}",
...
impl Debug for StrValidator<'_, '_> {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        write!(f, "{}", &self)
    }
}
...
mod test {
    ...
    fn test_input_builder() -> Result<(), Box<dyn Error>> {
        ...
        let less_than_1990_input = StrValidatorBuilder::default()
            .validators(vec![&less_than_1990])
            .build()?;

        let yyyy_mm_dd_input = StrValidatorBuilder::default()
            .validators(vec![&ymd_check])
            .build()?;

        let yyyy_mm_dd_input2 = StrValidatorBuilder::default()
    ...
    fn test_thread_safety() -> Result<(), Box<dyn Error>> {
        ...
        let ymd_input = StrValidatorBuilder::default()
            .validators(vec![&ymd_check])
            .build()?;
    ...
    /// Example showing shared references in `StrValidator`, and user-land, controls.
    #[test]
    ...
    fn test_validate_and_filter_detailed() {
        let input = StrValidatorBuilder::default()
            .required(true)
            .validators(vec![&less_than_1990])
            .filters(vec![&to_last_date_of_month])
            .build()
    ...
    fn test_value_type() {
        ...
        let _input = StrValidatorBuilder::default()
    ...
    fn test_display() {
        ...
        assert_eq!(
            input.to_string(),
            "StrValidator { break_on_failure: false, min_length: None, max_length: None, pattern: None, required: false, validators: Some([Validator; 1]), filters: None }",
        );
    }
}
